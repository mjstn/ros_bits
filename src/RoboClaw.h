...
#define CONNECT_MODE_USB        0
#define CONNECT_MODE_PACKET     1

#define PACKET_SER_ADDRESS	0x80
#define PACKET_ACK_BYTE		0xff
#define PACKET_MAX_LEN 		0x7e
#define PACKET_RETRIES 		15
#define PACKET_PER_BYTE_RETRIES 15 
#define PACKET_RETRIES_USEC	1000

// PACKETSerial is for packet serial comm on a serial device
class PACKETSerial {
public:
  class Exception : public std::runtime_error {
  public:
  ...
  PACKETSerial(uint8_t address);
  void Crc_update (uint16_t &crc, uint8_t data);
  void Open(const char *port, speed_t baudRate);
  bool IsOpen();
  void Close();
  uint8_t GetAddress();
  int Write(const unsigned char* data, int len);
  int WritePacket(const unsigned char *data, int len);
  int Read(char *buffer, int size, int timeoutMsec);
  int Flush();
private:
  uint8_t read_buf_[256];
  int buf_start_, buf_end_;
...
public:
  ...
  void SetEncM1(uint8_t address, uint32_t value);
  void SetEncM2(uint8_t address, uint32_t value);
  ...
  void setSerial(PACKETSerial *ser);

private:
  uint32_t connectMode_;
  USBSerial *usbSer_;
  PACKETSerial *packetSer_;
  ...
  // Send bytes from a prepaired buffer
  void write_buff(uint8_t byteCnt, uint8_t *buff);
